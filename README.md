# Daemonic - macOS Menubar Background Process Manager
...
```bash
curl -fsSL https://raw.githubusercontent.com/aolkin/daemonic/main/install.sh | bash
```
...
3. **Important**: Remove the quarantine attribute to avoid "app is damaged" error:
   ```bash
   xattr -cr /path/to/Daemonic.app
   ```
4. Move `Daemonic.app` to your Applications folder
...
- **Config location**: `~/.config/daemonic/config.json`
- **tmux prefix**: `daemonic-<command-id>`
